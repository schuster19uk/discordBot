    ...
    async execute(message) {
        ...
        try {
            ...
            const schedule = {
                ...
                5: ["15:00", "16:00", "17:00", "18:00", "19:30", "20:30", "21:30", "22:00"] // Fri
            };
                    ...
                    for (const timeStr of schedule[dayOfWeek]) {
                        ...
                        await conn.query(
                            `INSERT IGNORE INTO booking_slots 
                             (start_time, end_time, uk_time_display, nevada_time_display, is_available) 
                             VALUES (?, ?, ?, ?, TRUE)`,
            ...
            message.reply("✅ Schedule restocked with Dual-Timezone display columns!");

        } catch (err) {
            console.error(err);
            message.reply("❌ Error generating schedule.");
        } finally {
